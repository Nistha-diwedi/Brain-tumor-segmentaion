...
class BrainTumorClassifier():
    ...
    def restore_model(self, path):
        ...
        if os.path.exists(path):
            ...
            print(f"Model restored successfully from {path} on {self.device}")
        else:
    ...
    def test(self, testloader, threshold=0.5):
        ...
        # Convert DataLoader to iterator correctly
        testloader_iter = iter(testloader)

        # Running the loop until no more data is left to test.
        while len(test_data_indexes) != 0:
            try:
                # Correct way to fetch next batch
                data = next(testloader_iter)
            ...
            # Calculating the dice score for original and constructed image mask.
